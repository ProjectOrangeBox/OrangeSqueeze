...
class Lex extends ParserAbstract implements ParserInterface
{
	 ...
	 * @return  string
	 */
	public function lex_parse($text, $data = array(), $callback = false, $allow_php = false)
	{
		$this->setup_regex();
		...
		if (Lex::$data === null) {
			...
			$data = array_merge(Lex::$data, $data);
		...
		// set $cumulative_noparse to true and use Lex::inject_noparse($text); immediately
	...
	public function parse_callback_tags($text, $data, $callback)
	{
		...
		while (preg_match($regex, $text, $match, PREG_OFFSET_CAPTURE)) {
			...
			if (isset($match[2])) {
				$cb_data = $data;
				if (!empty(Lex::$callback_data)) {
					$cb_data = array_merge(Lex::$callback_data, (array) $data);
				}
	...
	public function parse_recursives($text, $orig_text, $callback)
	{
		...
		if (preg_match($this->recursive_regex, $text, $match)) {
			...
			$tag = $match[0];
			$next_tag = null;
			$children = Lex::$callback_data[$array_key];
	...
	{
		if (isset(Lex::$extractions['noparse'])) {
			foreach (Lex::$extractions['noparse'] as $hash => $replacement) {
	...
	protected function process_condition_var($match)
	{
		...
		if (
			in_array(strtolower($var), array('true', 'false', 'null', 'or', 'and')) or
	...
	protected function create_extraction($type, $extraction, $replacement, $text)
	{
		...
		Lex::$extractions[$type][$hash] = $replacement;
	...
	{
		if ($type === null) {
			foreach (Lex::$extractions as $type => $extractions) {
				foreach ($extractions as $hash => $replacement) {
					if (strpos($text, "{$type}_{$hash}") !== false) {
						$text = str_replace("{$type}_{$hash}", $replacement, $text);
						unset(Lex::$extractions[$type][$hash]);
					}
				}
			}
		} else {
			if (!isset(Lex::$extractions[$type])) {
				return $text;
			}

			foreach (Lex::$extractions[$type] as $hash => $replacement) {
				if (strpos($text, "{$type}_{$hash}") !== false) {
					$text = str_replace("{$type}_{$hash}", $replacement, $text);
					unset(Lex::$extractions[$type][$hash]);
				}
