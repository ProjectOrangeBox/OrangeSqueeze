...
use FS;
use Exception;
use projectorangebox\ci_parser\Ci;
...
class CiPlus extends Ci implements ParserInterface
{
	...
	protected function _ci_parse($template, $data)
	{
		...
		foreach ($data as $key => $val) {
			$merge = is_object($val) ? $this->_parse_object($key, $val, $template) : (is_array($val) ? $this->_parse_pair($key, $val, $template) : $this->_parse_single($key, (string) $val, $template));
	 ...
	 * @return string
	 */
	protected function _parse_object($key, $val, $template): array
	{
		$replace = array();
	...
	/**
	 * Replaced unparsed tags with a Parser dedicated tag (%EMPTY_VAR%)
	 * @protected
